...
function SaveCard() {
    if (document.getElementById('titleValue').value != '') {
        ...
        localStorage.setItem('TODOList', JSON.stringify(list.filter(item => item.id != id)));
...
cards.onclick = function(event) {
    ...
    if (parhId.includes('done')) {
        ...
        (document.getElementById(parhId).innerHTML == 'done') ? (document.getElementById(parhId).innerHTML = 'open') : (document.getElementById(parhId).innerHTML = 'done');
        markAsDoneCard(id)
    }
